...
export const clonerCarte = ({ carte, camp }) => {
    return {
        ...
        auraUnique: carte.auraUnique ? (cartesBoard, draggedCard) => carte.auraUnique(cartesBoard, draggedCard) : null,
        ...
        lancerSort: carte.lancerSort ? (setter, gold, board, shop, setterDeck, deck, setterShop) => carte.lancerSort(setter, gold, board, shop, setterDeck, deck, setterShop) : null,
    };
}
...
export const acheterCarte = ({ card, gold, setGold, deck, setDeck, shopCards, setShopCards, lvlTaverne, actualPlayer, goldPlayer2, deckPlayer2, setGoldPlayer2,setDeckPlayer2, setPopupMessage }) => {
  ...
  if(actualPlayer === 2){
    if(card.famille === "Sort" && goldPlayer2 < card.lvl){
      setPopupMessage("💰 Pas assez d'or !");
      return;
    }
    if (card.famille !== "Sort" && goldPlayer2 < 3) {
      setPopupMessage("💰 Pas assez d'or !");
      return;
    }
    ...
    if(card.famille === "Sort"){
      setGoldPlayer2(goldPlayer2 - card.lvl);
    }else{
