    ...
    if (sourceType === "deck" && targetType === "board-drop" && boardPlayer.length <7) {
        ...
        if(draggedCard.piocherCarteSousFamille){
            if(draggedCard.nom === "Maeva"){
                if(carteRandomSousFamille.nom === "Maya-Bull"){
                    carteRandomSousFamille.provocation = true
                    carteRandomSousFamille.provocationUse = true
                    carteRandomSousFamille.hp += 10
                    carteRandomSousFamille.buffHp += 10
                }
                if(carteRandomSousFamille.nom === "Titi L'aigri"){
                    carteRandomSousFamille.degatsAdj = true
                    carteRandomSousFamille.atk += 10
                    carteRandomSousFamille.buffAtk += 10
                }
            }
            const clone = clonerCarte({ carte: carteRandomSousFamille, camp: "joueur" });
            setDeck([...nouveauDeck, clone]);
            setTimeout(() => {
                fusionnerCartesIdentiques({
                    carteBase: draggedCard,
...
export function piocherCarteApresDecompte( decompteCard, deck, setDeck, boardPlayer, setBoardPlayer, setFusionAnim ){
    if(decompteCard.nom === "Lohan"){
        let piocheDoudou = cards.filter(card => card.sousFamille === "Doudou")
        ...
        let piochePets = cards.filter(card => card.sousFamille === "Les Pets")
        const randomCard = piochePets[Math.floor(Math.random() * piochePets.length)]
        if(randomCard.nom === "Maya-Bull"){
            randomCard.provocation = true
            randomCard.provocationUse = true
            randomCard.hp += 10
            randomCard.buffHp += 10
        }
        if(randomCard.nom === "Titi L'aigri"){
            randomCard.degatsAdj = true
            randomCard.atk += 10
            randomCard.buffAtk += 10
        }
        const clone = clonerCarte({ carte: randomCard, camp: "joueur" });
        setDeck([...deck, clone]);
        setTimeout(() => {
