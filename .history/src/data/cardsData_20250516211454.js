...
export const cards = [
  ...
  },
  {
    ...
    famille: "Croc-Noir",
    sousFamille : "Marin",
    texte: "<color=rgb(54, 132, 198)>Aura bivalente</color> <color=red>+4/+2</color> <br><br> <color=rgb(90, 61, 40)>Aura bivalente</color> <color=red>+2/+4</color> <br><br><br> Si Rok'gar est présent<br><br> Darka gagne <color=red>+4/+2</color>",
    effetDeCouple: {
  ...
  },
  {
    ...
    famille: "Croc-Noir",
    sousFamille : "Marin",
    texte: "<i>Cri</i><br><br> <color=red>+2/+2</color> à un Croc-Noir <br><br><br> <color=rgb(54, 132, 198)>Bivalence</color> <color=red>+2 atk</color> supp<br><br> <color=rgb(90, 61, 40)>Bivalence</color> <color=red>+2hp</color> supp",
    criDeGuerreUnique: (carte, draggedCard) => {
      if(draggedCard.estDoree){
        carte.atk += 2 * 2;
        carte.buffAtk +=2 * 2;
  ...
  },
  {
    ...
    famille: "Croc-Noir",
    sousFamille : "Terrestre",
    texte: "<br><color=rgb(20, 33, 220)>Aura</color> <color=red>+1/+2</color> aux Croc-Noir <br><br><br> <color=rgb(54, 132, 198)>Bivalence</color> <color=red>+2atk</color> <br><br><color=rgb(90, 61, 40)>Bivalence</color> <color=red>+2hp</color>",
    aura: (cartesBoard, draggedCard) => {
      if(draggedCard.estDoree){
        cartesBoard.forEach(carte => {
          if (carte.famille === "Croc-Noir") {
...

  {
    ...
    famille: "Les Sylphariels",
    sousFamille : "Light",
    texte: "S<i>Cri</i><br><br>Si seule gagne <color=red>+2atk</color><br><br><i>Evanescence(vente)</i><br><br><color=gold>+1 po</color>",
    gainOr: 1,
    criDeGuerreUniqueSelf: (carte, board) => {
      if(carte.estDoree){
        if(board.length === 0){
  ...
  },
  {
    ...
    famille: "Sort",
    texte: "<br><i>Lien Spirituel</i> <br><br><br>Vous obtenez une catre de la <br><br>famille et du lvl inferieur <br><br>à votre carte la plus à gauche",
    lancerSort: (setter, gold, board, shop, setterDeck, deck, setterShop) => {
  ...
  },
  {
    ...
    effetDeMass: (carte, board) => {
      if(carte.estDoree){
        const lesHierels = board.filter(c => c.famille === "Les Hierels" && c.id !== carte.id);
        if (lesHierels.length > 0) {
          carte.atk += lesHierels.length * 2;
          carte.hp += lesHierels.length * 2;
          carte.buffAtk += lesHierels.length * 2;
          carte.buffHp += lesHierels.length * 2; 
        }
        
        const lesZ = board.filter(c => c.sousFamille === "Les Z" && c.id !== carte.id);
        if (lesZ.length > 0) {
          carte.atk += lesZ.length * 2;
          carte.hp += lesZ.length * 2;
          carte.buffAtk += lesZ.length * 2;
          carte.buffHp += lesZ.length * 2; 
        }
      }else{
        const lesHierels = board.filter(c => c.famille === "Les Hierels" && c.id !== carte.id);
        if (lesHierels.length > 0) {
          carte.atk += lesHierels.length;
          carte.hp += lesHierels.length;
          carte.buffAtk += lesHierels.length;
          carte.buffHp += lesHierels.length; 
        } 
        const lesZ = board.filter(c => c.sousFamille === "Les Z" && c.id !== carte.id);
        if (lesZ.length > 0) {
          carte.atk += lesZ.length;
          carte.hp += lesZ.length;
          carte.buffAtk += lesZ.length;
          carte.buffHp += lesZ.length; 
        }
      }
